...
bestIndicesR=[];OptimalFIMADPRIndexR=[]; B=[]; I=[];
...
[x,fval,exitflag] = ga(@FIMADPR_GAFucFS,nVar,C,D,[],[],...
                       lb,ub,[],IntCon,opts);

bestIndicesR(j,:)=[x];
OptimalFIMADPRIndexR(j)=abs(fval);       
end

[B,I]= sort(OptimalFIMADPRIndexR);
bestIndicesFIMADPR_FSGA{SelectedSN,1} = bestIndicesR(I(1,end),:)
optimalFIMADPR_FSGA(SelectedSN,1) = B(1,end)
...
%% ======== Save the results  =============================================
save('OptResFIMADPRGAFS','optimalFIMADPR_FSGA','bestIndicesFIMADPR_FSGA');
    
%% ======== Function  =====================================================
function MEd = FIMADPR_GAFucFS(x)
    load Coe4Modes.mat
