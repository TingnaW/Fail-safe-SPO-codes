...
[SSCADPRFSEH{1,NumSen},LoopTSSCADPRFSEH(NumSen,1)] = ExhaustiveS_SSCADPRFSFuc(NumSen,C,CombN,CoeModeS,Y,NF);

[bestf, bestidx] = max(SSCADPRFSEH{1,NumSen});    
optimalSSCADPRFSEH(NumSen,1) = bestf
bestIndicesSSCADPRFSEH{NumSen,1} = C(bestidx,:)

AxisX=[];
AxisX = 1:CombN;
figure(NumSen+30)
plot(AxisX,SSCADPRFSEH{1,NumSen})
xlabel('Combination number')
ylabel('SSC')
set(gca,'FontSize',12)

[MaxK, Indall] = maxk(SSCADPRFSEH{1,NumSen},10);
...
%% ======== Save the results  =============================================
save('OptResSSCADPRExhFS','optimalSSCADPRFSEH','bestIndicesSSCADPRFSEH','LoopTSSCADPRFSEH','SSCADPRFSEH');

%% ======== Function  ===================================================== 
function [MEd,TimeP] = ExhaustiveS_SSCADPRFSFuc(NumSen,C,CombN,CoeModeS,Y,NF)
tic
