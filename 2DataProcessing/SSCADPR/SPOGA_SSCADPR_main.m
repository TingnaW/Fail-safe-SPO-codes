...
bestIndices=[];optimalSSCADPRIndex=[]; B=[]; I=[];
...
[x,fval,exitflag] = ga(@SSCADPR_GAFuc,nVar,C,D,[],[],...
                       lb,ub,[],IntCon,opts);

bestIndices(j,:)=[x];
optimalSSCADPRIndex(j)=abs(fval);    
end

[B,I]= sort(optimalSSCADPRIndex);
bestIndicesSSCADPRGA{SelectedSN,1} = bestIndices(I(1,end),:)
optimalSSCADPRGA(SelectedSN,1) = B(1,end)
...
%% ======== Save the results  =============================================
save('OptResSSCADPRGA','optimalSSCADPRGA','bestIndicesSSCADPRGA');

%% ======== Function  ===================================================== 
function MEd = SSCADPR_GAFuc(x)
