...
bestIndicesFS=[];optimalSSCIndexFS=[]; B=[]; I=[];
...
[x,fval,exitflag] = ga(@SSC_GAFucFS,nVar,C,D,[],[],...
                       lb,ub,[],IntCon,opts);

bestIndicesFS(j,:)=[x];
optimalSSCIndexFS(j)=abs(fval);    
end

[B,I]= sort(optimalSSCIndexFS);
bestIndicesSSCFSGA{SelectedSN,1} = bestIndicesFS(I(1,end),:)
optimalSSCFSGA(SelectedSN,1) = B(1,end)
...
%% ======== Save the results  =============================================
save('OptResSSCGAFS','optimalSSCFSGA','bestIndicesSSCFSGA');

%% ======== Function  =====================================================
function MEd = SSC_GAFucFS(x)
