...
[FIMADPR_FSREH{1,NumSen},LoopTFIMADPR_FSREH(NumSen,1)] = ExhaustiveS_FIMADPRFSFuc(NumSen,C,CombN,Coe4ModeM,NF);

[bestf, bestidx] = max(FIMADPR_FSREH{1,NumSen});    
optimalFIMADPR_FSREH(NumSen,1) = bestf
bestIndicesFIMADPR_FSREH{NumSen,1} = C(bestidx,:)

AxisX=[];
AxisX = 1:CombN;
figure(NumSen+40)
plot(AxisX,FIMADPR_FSREH{1,NumSen})
xlabel('Combination number')
ylabel('Weighted determinant of FIM')
set(gca,'FontSize',12)

[MaxK, Indall] = maxk(FIMADPR_FSREH{1,NumSen},10);
...
%% ======== Save the results  =============================================
save('OptResFIMADPRExhFSR','optimalFIMADPR_FSREH','bestIndicesFIMADPR_FSREH','LoopTFIMADPR_FSREH','FIMADPR_FSREH');
